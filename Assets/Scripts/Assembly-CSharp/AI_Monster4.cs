...
public class AI_Monster4 : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		if (!Check_LastPoint)
		{
			...
			if (gameObject2 != null)
			{
				if (!base.gameObject.GetComponent<FieldOfView4>().canSeePlayer)
				{
		...
		else
		{
			...
			if (FindClosestTarget(Players, base.transform) != null && base.gameObject.GetComponent<FieldOfView4>().canSeePlayer)
			{
				Shake.SetActive(true);
				Shake2.SetActive(true);
				animation.SetBool("stay", false);
