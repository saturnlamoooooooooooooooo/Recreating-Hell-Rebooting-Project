...
namespace UnityStandardAssets.Characters.FirstPerson
{
	...
	public class MyFPController : MonoBehaviour
	{
		public bool _startStaminaRegen;

		public double _counterStaminaResistance;
		...
		public double StaminaDowngrade;

		public double StaminaUpgrade;

		public double StaminaResistance;
		...
		private void Update()
		{
			...
			{
				_startStaminaRegen = true;
			}
			if (Input.GetKeyDown(KeyCode.LeftShift) && _counterStaminaResistance <= 0.0)
			{
				_startStaminaRegen = false;
				_counterStaminaResistance = StaminaResistance;
			}
			if (!m_IsWalking && _currentStamina >= StaminaDowngrade)
			{
				_currentStamina -= StaminaDowngrade;
			}
			else if (_startStaminaRegen)
			{
				if (_counterStaminaResistance > 0.0)
				{
					_counterStaminaResistance -= Time.deltaTime;
				}
				else if (_currentStamina < Stamina)
				{
					_currentStamina += StaminaUpgrade;
				}
			}
		}
		...
		private void GetInput(out float speed)
		{
			...
			speed = (m_IsWalking ? m_WalkSpeed : ((_currentStamina >= StaminaDowngrade) ? m_RunSpeed : m_WalkSpeed));
