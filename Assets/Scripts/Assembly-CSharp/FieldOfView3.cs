...
public class FieldOfView3 : MonoBehaviour
{
	...
	public void FixedUpdate()
	{
		...
		if (canSeePlayer)
		{
			base.gameObject.GetComponent<AI_Monster3>().AI_Enemy = AI_Monster3.AI_State.Chase;
		}
	}
			...
			if (!Physics.Raycast(base.transform.position, normalized, maxDistance, ObstacleMask))
			{
				...
				foreach (GameObject gameObject in array2)
				{
					...
					if (Vector3.Angle(base.transform.forward, normalized2) < angle / 2f)
					{
						...
						{
							base.gameObject.GetComponent<AI_Monster3>().AI_Enemy = AI_Monster3.AI_State.Stay;
						}
						else
						{
							canSeePlayer = false;
