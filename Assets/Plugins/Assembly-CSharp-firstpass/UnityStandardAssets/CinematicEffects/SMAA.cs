...
namespace UnityStandardAssets.CinematicEffects
{
	...
	public class SMAA : IAntiAliasing
	{
		...
		public void OnRenderImage(Camera camera, RenderTexture source, RenderTexture destination)
		{
			...
			material.SetVector(m_Metrics, new Vector4(1f / (float)pixelWidth, 1f / (float)pixelHeight, pixelWidth, pixelHeight));
			material.SetVector(m_Params1, new Vector4(qualitySettings.threshold, qualitySettings.depthThreshold, qualitySettings.maxSearchSteps, qualitySettings.maxDiagonalSearchSteps));
			material.SetVector(m_Params2, new Vector2(qualitySettings.cornerRounding, qualitySettings.localContrastAdaptationFactor));
			material.SetMatrix(m_ReprojectionMatrix, m_PreviousViewProjectionMatrix * Matrix4x4.Inverse(matrix4x));
			float num = ((m_FlipFlop < 0f) ? 2f : 1f);
			material.SetVector(m_SubsampleIndices, new Vector4(num, num, num, 0f));
			...
			else
			{
				...
				{
					material.SetTexture(m_BlendTex, renderTexture2);
					if (temporal.UseTemporal())
					{
