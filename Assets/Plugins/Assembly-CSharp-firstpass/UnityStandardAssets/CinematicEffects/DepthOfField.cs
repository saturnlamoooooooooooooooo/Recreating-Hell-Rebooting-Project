...
namespace UnityStandardAssets.CinematicEffects
{
	...
	public class DepthOfField : MonoBehaviour
	{
		...
		private bool shouldPerformBokeh
		{
			get
			{
				if (ImageEffectHelper.supportsDX11 && bokehTexture.texture != null)
				{
					return textureBokehMaterial;
				}
				return false;
			}
		}
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (settings.visualizeFocus)
			{
				...
				filmicDepthOfFieldMaterial.SetVector(m_BlurParams, blurParams);
				filmicDepthOfFieldMaterial.SetVector(m_BlurCoe, blurCoe);
		...
		private void DoDepthOfField(RenderTexture source, RenderTexture destination)
		{
			...
			if (shouldPerformBokeh)
			{
				...
				textureBokehMaterial.SetTexture(m_BlurredColor, temporaryRenderTexture3);
				textureBokehMaterial.SetFloat(m_SpawnHeuristic, bokehTexture.spawnHeuristic);
				textureBokehMaterial.SetVector(m_BokehParams, new Vector4(bokehTexture.scale * num2, bokehTexture.intensity, bokehTexture.threshold, num3));
			...
			filmicDepthOfFieldMaterial.SetVector(m_BlurParams, blurParams);
			filmicDepthOfFieldMaterial.SetVector(m_BlurCoe, value);
			...
			if (m_CurrentQualitySettings.dilateNearBlur)
			{
				...
				filmicDepthOfFieldMaterial.SetVector(m_Offsets, new Vector4(0f, num4 * 0.75f, 0f, 0f));
				Graphics.Blit(src, temporaryRenderTexture4, filmicDepthOfFieldMaterial, 2);
				filmicDepthOfFieldMaterial.SetVector(m_Offsets, new Vector4(num4 * 0.75f, 0f, 0f, 0f));
			...
			filmicDepthOfFieldMaterial.SetVector(m_BlurCoe, value);
			filmicDepthOfFieldMaterial.SetVector(m_Convolved_TexelSize, new Vector4(src.width, src.height, 1f / (float)src.width, 1f / (float)src.height));
			...
			if (shouldPerformBokeh)
			{
				...
				textureBokehMaterial.SetVector(m_Screen, new Vector3(1f / (1f * (float)source.width), 1f / (1f * (float)source.height), num3));
